...
| 卡组打印工作台 | YDK 到可编辑卡组与 A4 PDF；支持批量上传临时整卡图并直接打印；默认使用约 300 DPI 简中整卡，也可勾选约 600 DPI 高清重绘 |
| 图像识别 | 上传或摄像头拍摄单卡/多卡，复核后导出或继续编辑标准卡组 |
| 对局实验室 | 清晰简中手牌、可视化复数角色标记、精确概率、组合条件、失败诊断与换备 |
...
<table>
  <tr>
    <td><img src=".github/assets/recognition.jpg" alt="图像识别"></td>
...
- 原始识别图片和切片不会写入卡组备份。
- 排版预览与默认打印使用 680 × 986 简中整卡；高清重绘按需启用，生成更慢。
- 临时整卡图按原图进入预览与 PDF，并随 `.ygoproject` 保存，不套卡框或二次重绘。
- 首期识别面向规则截图、单卡照片和无遮挡平铺卡组；严重反光、遮挡或散乱堆叠需要人工复核。
- 标准 YDK/YDKe 只能保存官方数字卡号，原创卡与临时整卡图保存在 Better YGO 卡组备份中。
